...
- **测试覆盖**：284 个单元测试，涵盖直接转换器（Chat ↔ Anthropic、Responses ↔ Chat、Responses ↔ Anthropic）、thinking、指标、token 估算、使用量记录、SSE 解析、熔断器、启动 Banner、以及持久化存储的 schema 容错
...
这是本项目的**核心能力**。OpenProxyRouter 现在把三个公开客户端协议面都当作一等公民，按协议对直接互转：

- OpenAI Chat Completions ↔ Anthropic Messages
- OpenAI Responses ↔ OpenAI Chat Completions
- OpenAI Responses ↔ Anthropic Messages

路由仍然只由 `model` 决定；网关根据命中的后端协议选择对应的直接转换器。

| 客户端协议 | 后端类型 | 处理方式 | 典型场景 |
|:---------:|:---------:|:--------:|:--------|
| **OpenAI Chat** `/v1/chat/completions` | `anthropic` | **直接转换**（Chat ↔ Anthropic） | OpenAI SDK 调用 Claude |
| **OpenAI Chat** `/v1/chat/completions` | `openai` | 直传 | OpenAI SDK 调用 GPT / DeepSeek |
| **OpenAI Responses** `/v1/responses` | `openai` | **直接转换**（Responses ↔ Chat） | Codex / OpenAI SDK (Responses) 调用 GPT / DeepSeek |
| **OpenAI Responses** `/v1/responses` | `anthropic` | **直接转换**（Responses ↔ Anthropic） | Codex / OpenAI SDK (Responses) 调用 Claude |
| **Anthropic Messages** `/v1/messages` | `anthropic` | 直传 | Claude Code Desktop 调用 Claude |
| **Anthropic Messages** `/v1/messages` | `openai` | **直接转换**（Anthropic ↔ Chat） | Claude Code Desktop 调用 GPT |

跨协议能力覆盖：

| 能力 | Chat ↔ Anthropic | Responses ↔ Chat | Responses ↔ Anthropic |
|------|:------------------:|:----------------:|:---------------------:|
| 基本文本轮次 | ✅ | ✅ | ✅ |
| System / developer 指令 | ✅ | ✅ | ✅ |
| 流式 SSE | ✅ | ✅ | ✅ |
| Tool calls / function calling | ✅ | ✅ | ✅ |
| Tool results / function_call_output 历史 | ✅ | ✅ | ✅ |
| 图片输入（base64 / URL） | ✅ | ✅ | ✅ |
| stop / finish / incomplete 映射 | ✅ | ✅ | ✅ |
| Temperature / top_p / max token 控制 | ✅ | ✅ | ✅ |
| Usage 统计（含缓存 token） | ✅ | ✅ | ✅ |
| Thinking / reasoning 归一化 | ✅ | ✅ | ✅ |
...
# 用 Responses API 调用 Claude（Responses ↔ Anthropic 直接转换）
...
# 6. OpenAI Responses 协议 → Anthropic 后端（Responses ↔ Anthropic 直接转换）
curl -k https://127.0.0.1:8443/anthropic/v1/responses \
  -H 'Content-Type: application/json' \
...
（`response.created` → `response.output_text.delta` → `response.completed` 等）。对于 OpenAI 兼容后端，
它直接在 Responses 与 Chat Completions 之间转换；对于 Anthropic 后端，它直接在 Responses items
与 Anthropic content blocks 之间转换。
...
许多 Chat 兼容 reasoning 后端会用 `reasoning_content` 暴露流式 thinking；网关会尽量通过该字段保留 Anthropic `thinking` 块。Responses 客户端则会收到 `reasoning` output item。若上游本身不暴露独立 reasoning 字段，这部分数据可能缺失。
...
A：网关会把 Anthropic `thinking` 块映射为 Chat 的 `reasoning_content`（当客户端/后端支持时），在 `/v1/responses` 上则映射为 Responses 的 `reasoning` output item。请求侧的 reasoning/thinking 参数会按后端 `thinking_format` 归一化。
...
│   ├── converters.js      # Chat ↔ Anthropic 转换器 + SSE 流翻译器 + parseAnthropicSSEUsage
│   ├── converters_responses.js
│   │                       # Responses ↔ Chat 转换器 + SSE 流翻译器
│   ├── converters_responses_anthropic.js
│   │                       # Responses ↔ Anthropic 直接转换器 + SSE 流翻译器
│   ├── handlers.js        # Chat/Messages 代理处理器 + Token 用量采集
│   ├── handlers_responses.js
│   │                       # Responses 到 OpenAI / Anthropic 后端的代理处理器
